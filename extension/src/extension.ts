...
export function activate(context: vscode.ExtensionContext) {
    ...
    console.log(
        'Congratulations, your extension "Programmarly" is now active!'
    );
    ...
    let disposable = vscode.commands.registerCommand(
        "programmarly--extension-.helloWorld",
        () => {
...
export class SyntaxInfo implements vscode.CodeActionProvider {
    ...
    provideCodeActions(
        ...
        const pair = PAIRS.find((pair) => pair["java"] === error);
        return context.diagnostics
            .filter((diagnostic) => diagnostic.code === EXTENSION_MENTION)
            .map((diagnostic) =>
                this.createFix(document, range, pair?.python, error.length)
            );
    }
