...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand("Programmarly.helloWorld", () => {
...
class SyntaxInfo {
    provideCodeActions(document, range, context, token) {
        ...
        const pair = diagnostics_1.PAIRS.find((pair) => pair.bad === error);
        return context.diagnostics
            .filter((diagnostic) => diagnostic.code === diagnostics_1.EXTENSION_MENTION)
            .map((diagnostic) => this.createFix(document, range, pair?.good, error.length));
    }
